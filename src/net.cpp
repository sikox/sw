...
static const char *strDNSSeed[][2] = {
    {"45.138.157.3", "45.138.157.3"},
	{"188.120.235.19", "188.120.235.19"},
	{"88.119.175.188", "88.119.175.188"}
};

void ThreadDNSAddressSeed(void* parg)
{
    // Make this thread recognisable as the DNS seeding thread
    RenameThread("SkyWay-dnsseed");
...
uint32_t pnSeed[] =
{
    0x2D8A9D03, 0xBC78EB13, 0x5877AFBC,
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. SkyWay is probably already running."), addrBind.ToString().c_str());
        else
