...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #SkyWayTEST2\r");
            Send(hSocket, "WHO #SkyWayTEST2\r");
        } else {
            // randomly join #SkyWay00-#SkyWay05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #SkyWay%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #SkyWay%02d\r", channel_number).c_str());
        }
